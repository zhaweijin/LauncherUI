...
public class PercentageBarChart extends View {
    ...
    private String TAG = "PercentageBarChart";
    ...
    public PercentageBarChart(Context context, AttributeSet attrs) {
        ...
        mMinTickWidth = a.getDimensionPixelSize(R.styleable.PercentageBarChart_minTickWidth, 10);
        int emptyColor = a.getColor(R.styleable.PercentageBarChart_emptyColor, context.getResources().getColor(R.color.memory_avail));
        a.recycle();
        Log.v(TAG, "INIT");
...
//                    Log.v(TAG, ">>>>>mMinTickWidth="+mMinTickWidth);
//                    Log.v(TAG, ">>>>>width="+width);
//                    Log.v(TAG, ">>>>>draw,width="+entryWidth);
//                    Log.v(TAG, ">>>>>paint="+e.paint.getColor());
                    final float nextX = lastX + entryWidth;
                    if (nextX > right) {
                        canvas.drawRect(lastX, top, right, bottom, e.paint);
                        return;
                    }
//                    Log.v(TAG, ">>>>>nextx="+nextX);
                    canvas.drawRect(lastX, top, nextX, bottom, e.paint);
                    lastX = nextX;
                }
            }
